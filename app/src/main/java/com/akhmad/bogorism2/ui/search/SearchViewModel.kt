...
class SearchViewModel: ViewModel() {
    ...
    private val _itemList = MutableLiveData<List<PlaceEntity>>()
    private val _isLoading = MutableLiveData<Boolean>()
    private val _toastText = MutableLiveData<Load<String>>()

    val searchModel: LiveData<List<PlaceEntity>> = _itemList
        ...
        client.enqueue(object : Callback<SearchResponse> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    _itemList.value = response.body()?.items as List<PlaceEntity>
                } else {
